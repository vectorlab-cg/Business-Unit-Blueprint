...
(function (global) {
  ...
  function genera(bu) {
    ...
    righe.push('_Chi siete e cosa fate, in apertura._');
    righe.push(bu.nome + ': ' + render.testoCampo(bu, 'identita', 'descrizione'));
    righe.push('');
    righe.push('## Slide 2 — Il problema');
    righe.push('_Le leve, per far riconoscere a chi ascolta il problema che ha davvero._');
    righe.push(slideProblema(bu));
    righe.push('');
    righe.push('## Slide 3 — La soluzione');
    righe.push('_Come lo risolvete, in concreto._');
    righe.push(render.testoCampo(bu, 'identita', 'meccanismo'));
    righe.push('');
    righe.push('## Slide 4 — Prove');
    righe.push('_Casi reali, numeri, riferimenti — solo se esistono davvero._');
    righe.push(slideProve());
    righe.push('');
    righe.push('## Slide 5 — Offerta');
    righe.push('_Cosa include, cosa costa, cosa promette._');
    righe.push(slideOfferta(bu));
    righe.push('');
    righe.push('## Slide 6 — Chi siamo');
    righe.push('_Le persone del team: dà credibilità a chi valuta se affidarvi il progetto._');
    righe.push(slideChiSiamo(bu));
    righe.push('');
    righe.push('## Slide 7 — Prossimi passi');
    righe.push('_Cosa deve succedere subito dopo questa presentazione._');
